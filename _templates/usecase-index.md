...
*Nguồn thật ở `{{feature}}-usecase-diagram.puml` (PlantUML native), render bằng `node .agents/scripts/plantuml-render.mjs` ra `.svg`. Sửa nội dung → sửa `.puml` rồi gọi lại `$usecase-diagram`, KHÔNG sửa tay `.svg`.*
