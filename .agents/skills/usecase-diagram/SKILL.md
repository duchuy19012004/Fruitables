...
description: Dùng khi cần use case diagram tổng quan actor và phạm vi chức năng bằng PlantUML. Trong Codex, gọi bằng `$usecase-diagram` kèm feature slug.
---

## Codex project adapter

- Gọi rõ bằng `$usecase-diagram`.
- Render bằng `node .agents/scripts/plantuml-render.mjs <file.puml> [--png]`; không phụ thuộc Bash, `python3` hay `curl` trên Windows.
- Nội dung sơ đồ được gửi tới máy chủ công khai `plantuml.com`; không dùng với dữ liệu nhạy cảm.
- Bỏ qua các bước `CLAUDE_*`/hook changelog nếu project chưa cấu hình hook tương ứng.

# $usecase-diagram — Use Case Diagram (PlantUML native)

## Goal

Produce visual use case diagram cho 1 feature: actors (bên ngoài system) + use cases (bên trong system boundary, gom nhóm theo package khi nhiều) + relationships (`include`, `extend`, `generalization`). Phục vụ kickoff stakeholder và system scope overview — KHÔNG phải đặc tả chi tiết từng luồng.
...
- **Chạy độc lập được** — nếu feature hoặc tài liệu nguồn chưa có, hỏi user về actors, mục tiêu/use case và relationship; derive slug; preview L1; chỉ tạo folder/index/diagram sau khi user duyệt. Không tự bịa dữ kiện còn thiếu.
- **Render qua `node .agents/scripts/plantuml-render.mjs`** (dùng chung script trong `.agents/skills/usecase-diagram/`) — KHÔNG tự gọi curl/encode trực tiếp trong skill logic, script lo hết.
...
Trước khi làm, dùng công cụ đọc/tìm file hiện có để:

- lấy ngày hiện tại từ môi trường;
- liệt kê feature và artifact liên quan dưới `docs/` (ưu tiên `rg --files docs`);
- đọc SRS, use case hoặc diagram hiện có trước khi hỏi user;
- kiểm tra engine bằng lệnh project-local ghi trong phần **Codex project adapter**.

Không coi cú pháp macro `!` của Claude Code là lệnh tự chạy trong Codex.
## Approach

1. **Resolve feature** — `--feature` explicit nếu có; else auto-detect (single in-progress) hoặc picker.
   - **Feature/nguồn đã có** → đọc và tái sử dụng actor/use case đã ghi; không hỏi lại dữ kiện đã biết.
   - **Feature hoặc nguồn chưa có** → coi đây là điểm vào: lấy mô tả hiện tại, derive slug kebab-case, hỏi ngắn gọn actors + mục tiêu/use case + relationship có bằng chứng. Không suy được slug rõ thì hỏi user. Preview slug/nội dung ở L1; sau khi user duyệt mới tạo `docs/{feature}/usecases/` và index từ template.
...
3. **Auto-detect actors + use cases:**
   ...
   - Nếu chưa có nguồn, dùng câu trả lời elicitation ở bước 1; chỗ chưa rõ phải hỏi hoặc đánh dấu cần xác nhận, không bịa.
...
9. **Write** `docs/{feature}/usecases/{feature}-usecase-diagram.puml` → chạy `node .agents/scripts/plantuml-render.mjs` → sinh `{feature}-usecase-diagram.svg`.
...
13. **Output report:**
    ...
    Nếu cần đặc tả chi tiết từng use case, hãy yêu cầu Codex viết main flow, alternate flow và error flow từ index này.
    ```
...
- **Feature/nguồn hoàn toàn không tồn tại** — chuyển sang elicitation ngắn theo bước 1, derive slug và chờ L1 approval trước khi tạo. KHÔNG bịa actor/use case.
- **Render qua server công khai (plantuml.com)** — nội dung diagram gửi qua internet mỗi lần render. Cần offline/không muốn gửi data ra ngoài → cài Java + PlantUML local hoặc dùng server nội bộ đã được phê duyệt, rồi thay adapter HTTP. Skill hiện tại không tự thay đổi cơ chế này.
...
- **Diagram ≠ detail flow, ≠ semantic validation.** Diagram chỉ show visual scope + relationship; detail steps nằm trong đặc tả use case riêng. Render PASS KHÔNG có nghĩa đúng nghiệp vụ — tự soi goal level (UC cùng level, ưu tiên sea), actor coverage, relationship đúng hướng + có rationale, tên UC khớp catalog `{feature}-usecase-index.md`.
...
- **Compile fail (HTTP non-200 hoặc SVG <200 bytes)** — `node .agents/scripts/plantuml-render.mjs` tự phát hiện, trả exit code khác 0. Đọc lỗi cụ thể, sửa `.puml` (thường do quote/alias thiếu), render lại. Tối đa 2 lần tự sửa trước khi báo user.
- **Preview đọc `{feature}-usecase-index.md`** — section `## Diagram` đã nhúng `<img src="{feature}-usecase-diagram.svg">` (HTML chuẩn) nên trình xem Markdown/Obsidian hiển thị ảnh bình thường. KHÔNG còn file `diagram.md` riêng.
...
- @../../scripts/plantuml-render.mjs (compile .puml → .svg qua plantuml.com)
