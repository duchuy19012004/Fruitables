---
name: dbdiagram
description: Dùng khi cần schema database DBML để import dbdiagram.io, dbdocs.io hoặc export SQL. Trong Codex, gọi bằng `$dbdiagram` kèm feature slug.
---

## Codex project adapter

- Gọi rõ bằng `$dbdiagram`.
- Validate/export bằng `npm exec --prefix .agents -- dbml2sql <file.dbml> --postgres -o <file.sql>`; không cài DBML CLI toàn máy.
- Bỏ qua preflight `command -v` kiểu Unix và các bước `CLAUDE_*`/hook changelog nếu project chưa cấu hình hook tương ứng.
...
User gọi $dbdiagram --feature X   (hoặc $dbdiagram "<mô tả data model>")
   │  dbml2sql chưa sẵn sàng? → chạy `& .\.agents\scripts\setup-tools.ps1`, rồi kiểm tra lại
...
```bash
npm exec --prefix .agents -- dbml2sql docs/{feature}/dbdiagram/{feature}.dbml --postgres -o docs/{feature}/dbdiagram/{feature}.sql
...
- **dbml2sql chưa sẵn sàng** → chạy `& .\.agents\scripts\setup-tools.ps1`, rồi thử lại. Dependency nằm trong project; không cần cài toàn máy. KHÔNG ghi file rồi bỏ mặc không validate.
