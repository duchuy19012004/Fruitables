...
description: Dùng khi cần activity diagram có swimlane thật cho quy trình đa vai trò bằng PlantUML. Trong Codex, gọi bằng `$activity-swimlane` kèm mô tả và feature slug.
---

## Codex project adapter

- Gọi rõ bằng `$activity-swimlane`.
...
- **Render qua plantuml.com** (`node .agents/scripts/plantuml-render.mjs`) — máy này KHÔNG có Java. **TRADE-OFF**: nội dung diagram (tên lane/bước) gửi qua internet mỗi lần render. Nội dung nhạy cảm → không dùng skill này (xem Gotchas).
- **AI viết source .puml, KHÔNG tự tính toạ độ** — PlantUML layout engine lo. Vai trò AI = mô tả đúng nghiệp vụ (lane/bước/nhánh/loop).
- **Compile phải PASS** trước khi báo xong. `node .agents/scripts/plantuml-render.mjs` bắt cả lỗi HTTP lẫn "Syntax Error" nhúng trong SVG. Fail → sửa source, render lại (tối đa 2 lần).
...
1. Resolve feature + process slug (verb-object kebab-case từ mô tả, max 40 chars)
   ...
   │  └─ không truy cập được plantuml.com → dừng, báo 1 dòng (xem Context). KHÔNG ghi file rỗng.
   ▼
...
6. L1 plan preview (prose BA-friendly: N bước, M decision, K lane). User Y → tiếp.
   ▼
7. Write .puml → node .agents/scripts/plantuml-render.mjs --png → SVG + PNG.
...
```bash
node .agents/scripts/plantuml-render.mjs docs/{feature}/srs/{feature}-{slug}-swimlane.puml --png
...
> Nguồn PlantUML: `{feature}-{slug}-swimlane.puml`. Sửa .puml → chạy `node .agents/scripts/plantuml-render.mjs` regen .svg.
```
...
- **Không truy cập được plantuml.com** → dừng ngay bước 1, in đúng 1 dòng. KHÔNG ghi file rỗng.
...
- **Syntax Error nhúng trong SVG** — plantuml.com trả HTTP 200 kèm ảnh chứa text lỗi. `node .agents/scripts/plantuml-render.mjs` đã grep bắt "Syntax Error" → exit non-0. Đừng bỏ qua.
...
- **Path ảnh trong flows.md** — viết tương đối với vị trí flows.md (ở `srs/`), tức `![...](./{feature}-{slug}-swimlane.svg)` (cùng thư mục). `/preview` và `/export` TỰ ĐỘNG xử lý ảnh này:
  ...
  - **Nên GIỮ file `.png` cạnh `.svg`** (node .agents/scripts/plantuml-render.mjs --png sinh cả 2) — export PDF/DOCX dùng lại PNG này, khỏi convert lại.
...
- @../../scripts/plantuml-render.mjs (render qua plantuml.com — bước 7)
