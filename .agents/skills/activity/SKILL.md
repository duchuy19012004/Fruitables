---
name: activity
description: Dùng khi cần vẽ activity/flowchart Mermaid cho quy trình nghiệp vụ có nhiều nhánh quyết định. Trong Codex, gọi bằng `$activity` kèm mô tả và feature slug.
---

## Codex project adapter

- Gọi rõ bằng `$activity`.
- Verify bằng `node .agents/scripts/mermaid-verify.mjs`; script dùng Mermaid CLI cục bộ và tự tìm Chrome trên Windows.
- Khi vượt ngưỡng phức tạp, spawn custom agent `diagram-reviewer` từ `.codex/agents/diagram-reviewer.toml`.
- Bỏ qua các bước `CLAUDE_*`/hook changelog nếu project chưa cấu hình hook tương ứng; không được báo đã ghi activity log khi chưa ghi thật.
...
9.5. **Render-verify (BẮT BUỘC, chạy ngay sau Write)** — `node .agents/scripts/mermaid-verify.mjs --file docs/{feature}/srs/{feature}-flows.md`. Mermaid không render trong chat (đây là lý do skip L3), nên đây là cách duy nhất bắt lỗi cú pháp TRƯỚC khi báo "xong" thay vì để user tự phát hiện khi mở IDE.
...
- @../../../.codex/agents/diagram-reviewer.toml (Diagram_Reviewer — review coverage khi vượt ngưỡng phức tạp, bước 9.7)
