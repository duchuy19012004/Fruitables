---
name: d2-activity
description: Dùng khi cần activity/flowchart đẹp đứng riêng bằng D2 và layout ELK. Trong Codex, gọi bằng `$d2-activity` kèm mô tả và feature slug.
---

## Codex project adapter

- Gọi rõ bằng `$d2-activity`.
- Render bằng `node .agents/scripts/d2-render.mjs <file.d2> [--png]`; không dùng preflight hoặc script Bash kiểu Unix.
- D2 binary cục bộ nằm trong `.agents/tools/d2` sau khi chạy setup.
...
> Họ skill D2: `$d2-activity` (flow này) · `$d2-erd` (data model) · `$d2-architect` (kiến trúc hệ thống). Cả 3 dùng chung `.agents/scripts/d2-render.mjs`.
...
- **Output cố định** `docs/{feature}/d2/{slug}.d2` + `.svg`. KHÔNG ghi vào `flows.md`. KHÔNG flag layout/theme/direction (đã chốt ELK + theme neutral trong `node .agents/scripts/d2-render.mjs`).
...
- **Render qua `node .agents/scripts/d2-render.mjs`** — KHÔNG gọi `d2`/Chrome trực tiếp trong skill (script lo path + flag).
...
5. L1 plan preview (prose BA-friendly: N bước, M nhánh, K lane). User Y → tiếp
   ▼
6. Write {slug}.d2 → chạy node .agents/scripts/d2-render.mjs → sinh {slug}.svg
...
```bash
node .agents/scripts/d2-render.mjs docs/{feature}/d2/{slug}.d2
...
- **PNG (khi cần export)** → `node .agents/scripts/d2-render.mjs {file}.d2 --png`. Mặc định chỉ SVG cho nhẹ; adapter dùng Chrome đã cài và kích thước thật từ `viewBox` để chụp PNG không bị crop.
...
- @../../scripts/d2-render.mjs (compile D2 thành SVG/PNG trên Windows)
