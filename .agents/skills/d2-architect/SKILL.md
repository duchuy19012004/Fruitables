---
name: d2-architect
description: Dùng khi cần sơ đồ kiến trúc hệ thống bằng D2 với component, service, database và dịch vụ ngoài. Trong Codex, gọi bằng `$d2-architect`.
---

## Codex project adapter

- Gọi rõ bằng `$d2-architect`.
...
> Họ skill D2: `$d2-activity` (flow) · `$d2-erd` (data model) · `$d2-architect` (kiến trúc này). Cả 3 dùng chung `.agents/scripts/d2-render.mjs`.
...
- **Render qua `node .agents/scripts/d2-render.mjs`** (dùng chung). KHÔNG gọi d2/Chrome trực tiếp.
...
4. L1 plan preview (prose: K khối, N service, M dịch vụ ngoài). User Y → tiếp
   ▼
5. Write {slug}.d2 → node .agents/scripts/d2-render.mjs → {slug}.svg (compile fail → sửa, tối đa 2 lần)
   ▼
...
```bash
node .agents/scripts/d2-render.mjs docs/{feature}/d2-architect/{slug}.d2
